...
void Move::generate_check_squares() {
    ...
    while (king_bitboard) {
        int initial_square = CTZ(king_bitboard);
        ...
        for (int direction = 0; direction < 4; direction++) {
            ...
            if (tmp_rook_blockers) {
                int attacked_square = direction < 2 ? CTZ(tmp_rook_blockers) : 63 - CLZ(tmp_rook_blockers);
                if (player_bitboard & (1ull << attacked_square)) {
                    ...
                    if (opponent_king_bitboard & (1ull << attacked_square)) {
                        if (tmp_rook_blockers) {
                            int next_attacked_square = direction < 2 ? CTZ(tmp_rook_blockers) : 63 - CLZ(tmp_rook_blockers);
    ...
    while (king_bitboard) {
        ...
        for (int direction = 0; direction < 4; direction++) {
            ...
            if (tmp_bishop_blockers) {
                int attacked_square = direction < 2 ? CTZ(tmp_bishop_blockers) : 63 - CLZ(tmp_bishop_blockers);
                if (player_bitboard & (1ull << attacked_square)) {
                    uint64_t tmp_points = two_points[initial_square][attacked_square];
                    ...
                    if (opponent_king_bitboard & (1ull << attacked_square)) {
                        if (tmp_bishop_blockers) {
                            int next_attacked_square = direction < 2 ? CTZ(tmp_bishop_blockers) : 63 - CLZ(tmp_bishop_blockers);
...
void Move::generate_queen_attacks() {
    ...
    while (current_queen_bitboard) {
        int initial_square = CTZ(current_queen_bitboard);
...
void Move::generate_rook_attacks() {
    ...
    while (current_rook_bitboard) {
        int initial_square = CTZ(current_rook_bitboard);
...
void Move::generate_bishop_attacks() {
    ...
    while (current_bishop_bitboard) {
        int initial_square = CTZ(current_bishop_bitboard);
...
void Move::generate_knight_attacks() {
    ...
    while (current_knight_bitboard) {
        int initial_square = CTZ(current_knight_bitboard);
...
void Move::generate_pawn_attacks() {
    ...
    while (current_pawn_bitboard) {
        int initial_square = CTZ(current_pawn_bitboard);
...
void Move::generate_king_attacks() {
    ...
    while (current_king_bitboard) {
        int initial_square = CTZ(current_king_bitboard);
